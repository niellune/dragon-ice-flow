...
1. **Report current state** — run `powershell -File .claude/hooks/budget-check.ps1 -All`, then `powershell -File .claude/scripts/usage.ps1 -By week` (main vs subagent consumption from the transcripts), then `powershell -File .claude/scripts/unslop/unslop.ps1 -All` (prose findings on the record roots; a true finding is a rewrite through the folder's gate, a false one is a rule fix), then `powershell -File .claude/scripts/apply-stack.ps1 -Check` when a stack pack is applied (rows the project changed on purpose are `drifted` by design; note them in STATE's Stack & Versions), then token counts of the other always-load files vs targets, file counts in each folder.
